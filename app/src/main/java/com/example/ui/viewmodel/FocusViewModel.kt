    ...
    init {
        ...
        viewModelScope.launch {
            ...
            schedules.first().let { currentScheds ->
                ...
                // Clear existing dummy "TikTok" schedules from previous app versions or automatic onboarding defaults
                currentScheds.forEach {
                    val isDummyTikTok = it.appName.equals("TikTok", ignoreCase = true) && (
                        it.todoWhileLocked == "Physical reading & deep restorative sleep prep" ||
                        it.todoWhileLocked == "Wind down screen-free: offline paper book, light stretching, deep breaths." ||
                        it.customAlertSms.contains("FocusFlow Alert: TikTok locked!", ignoreCase = true) ||
                        it.customAlertSms.contains("Late night sleep lockdown active!", ignoreCase = true)
                    )
                    if (isDummyTikTok) {
